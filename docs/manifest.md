...
| `profile` | 2 | `"factory"` or `"preserve"` | Default for every build. Absent means `factory`. |
...
| Field | Schema | Type | Meaning |
|---|---|---|---|
| `chipFamily` | 1 | string | One of the eleven families in the table below. Required. |
...
| `flashSizeMB` | 2 | integer 0–1024 | The build is offered only on a chip with exactly this flash size. |
| `usbVendorId`, `usbProductId` | 2 | integer 0–65535 | Filter the browser's port picker, and reject a board that reports different ids. |
...
| `profile`, `eraseAll` | 2 | as above | Per-build override of the top-level value. |
...
| `sha256` | 2 | 64 lowercase hex characters | Required by `preserve`. Without it the installer hashes the download anyway and writes the hash to the log. |

### `compatibility`, for the `preserve` profile

| Field | Type | Meaning |
|---|---|---|
| `regions` | array of `{offset, size, sha256}` | Flash regions that must already hold exactly these bytes, in both modes. |
| `firstInstall.regions` | same | Extra regions checked only in *first installation* mode. |
| `firstInstall.empty` | array of `{offset, size}` | Regions that must read back as all `0xff` in *first installation* mode. |
| `update.tableOffset` | integer ≥ 0 | Offset of the partition table. In *update* mode the page requires the sector there to hold this release's table, padded with `0xff`. There must be a part at exactly this offset. |

A region without `sha256` is refused at install time: it makes a claim the
installer cannot check, so it fails closed rather than passing silently. The
`preserve` profile also needs at least one region between `regions` and
`firstInstall.regions`, and it may not set `eraseAll`.
...
The tool refuses to write a manifest whose parts sit above it in the directory
tree, because a web server rarely mirrors that:
...
binaries elsewhere, for example on a release host. List the exact origins, scheme
and host and port:
...
Two things follow. The other host has to send permissive CORS headers, because
the page fetches the binaries with `fetch()`. And the origin check is repeated on
the *response*: redirects are followed, but if the bytes finally arrive from an
origin other than the one requested, the download is rejected.
...
Checks it runs: the Content-Security-Policy meta tag in `index.html`; the
checksum of the vendored esptool-js bundle against `vendor/esptool-js/SHA256SUMS`;
that the catalog lists systems and each system lists releases; release ordering;
manifest shape, schema, profile, board keys and duplicates; per part the served
size and SHA-256; part ordering and overlap; and the ESP image header of whatever
covers the chip's bootloader offset.
