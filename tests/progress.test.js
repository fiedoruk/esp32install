...
test('backupMinutes: about 1 MB a minute through a UART bridge, about 8 through the chip\'s own USB port, whole minutes, never zero', () => {
  assert.equal(backupMinutes({ flashSizeMB: 16, usbVendorId: 0x1a86 }), 16, 'CH9102 bridge');
  assert.equal(backupMinutes({ flashSizeMB: 16, usbVendorId: 0x10c4 }), 16, 'CP210x bridge');
  assert.equal(backupMinutes({ flashSizeMB: 16 }), 16, 'no vendor id reported: the slow assumption');
  assert.equal(backupMinutes({ flashSizeMB: 16, usbVendorId: 0x303a }), 2, 'Espressif USB-JTAG');
  assert.equal(backupMinutes({ flashSizeMB: 4, usbVendorId: 0x303a }), 1);
  assert.equal(backupMinutes({ flashSizeMB: 4 }), 4);
  assert.equal(backupMinutes({ flashSizeMB: 1 }), 1);
});
